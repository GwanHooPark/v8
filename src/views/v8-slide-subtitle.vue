  ...
  <div class="reveal">
    ...
    <div class="slides">
      ...
      <section data-title="Index">
        ...
        <p class="subtitle fragment fade-in-then-out">물론 이 팁을 적용하시더라도 현재 담당하시는 어플리케이션의 속대롤 대폭 향상시키진 않습니다.</p>
      </section>
      <section data-title="1.JS Engine">
        ...
        <p class="subtitle fragment fade-in-then-out">그리고 주로 웹브라우저를 위해 사용됩니다.</p>
      </section>
      <section data-title="1.JS Engine">
      ...
      <section data-title="2.JS Engine 작동원리">
        ...
        <p class="subtitle fragment fade-in-then-out">javascript엔진의 컴파일 흐름을 보기전에 먼저 최근 대부분의 javascript엔진에서 사용하는 컴파일 방식을 보겠습니다.</p>
        <p class="subtitle fragment fade-in-then-out">먼저 Just in Time Compilation이라고하는 방식입니다. jit컴파일이라고도 하는데요.</p>
        <p class="subtitle fragment fade-in-then-out">JITC의 수행과정을 보면 처음에 text형태로 입력된 소스코드를 파싱하여 bytecode로 변환합니다.</p>
        <p class="subtitle fragment fade-in-then-out">그리고 인터프리터모드면 bytecode를 하나씩 읽어가며 수행하고 JIT모드면 생성된 bytecode를 native code로 컴파일하여 수행합니다.</p>
        <p class="subtitle fragment fade-in-then-out">인터프리터로 bytecode를 수행하는것보다 native code로 수행하는것이 빠르지만</p>
        <p class="subtitle fragment fade-in-then-out">문제는 JITC은 컴파일과정이 수행중에 발생하기 때문에 이것 자체가 오버헤드의 원인이 됩니다.</p>
        <p class="subtitle fragment fade-in-then-out">그래서 컴파일에 많은 시간을 쓰지 못하고 아주 최소한의 최적화만 적용하여 native code를 생성합니다.</p>
      </section>
      <section data-title="2.JS Engine 작동원리">
        ...
        <p class="subtitle fragment fade-in-then-out">그래서 javascript JITC는 모든 예외적인 케이스를 다 고려하여 코드를 생성해야합니다.</p>
        <p class="subtitle fragment fade-in-then-out">그러면 많은 양의 native code가 생기게 되고 이는 결국엔 interpreter와 별다른 차이가 없다고 합니다.</p>
        <p class="subtitle fragment fade-in-then-out">게다가 bytecode를 nativecode로 변환하는 overhead까지 더해지니 비효율적이라고 할수 있습니다.</p>
        <p class="subtitle fragment fade-in-then-out">또 다른 문제는 Javascript는 연산을 많이하기 보다는 주로 DOM을 조작하거나 사용자의 입력에 반응하는 프로그램이 많습니다.</p>
        ...
        <p class="subtitle fragment fade-in-then-out">그러면 javascript에서는 JITC보다 interpreter가 더 좋은것이 아니냐고 할 수 있는데요.</p>
        <p class="subtitle fragment fade-in-then-out">최근에는 꼭 그렇다고만 할 수도 없습니다.</p>
        <p class="subtitle fragment fade-in-then-out">최근 javascript들은 단순히 웹페이지의 DOM조작이나 이벤트 처리뿐만 아니라 비즈니스로직에도 어느정도 관여하며 연산이 많은 프로그램들도 구현되고 있습니다.</p>
      ...
      <section data-title="2.JS Engine 작동원리">
        ...
        <p class="subtitle fragment fade-in-then-out">정리하면 최근의 자바스크립트엔진은 JITC 방식과 interpreter방식을 혼용해서 사용합니다.</p>
      ...
      <section data-title="2.JS Engine 작동원리(공통)">
        ...
        <p class="subtitle fragment fade-in-then-out">지금 보시는 순서도는 자바스크립트 엔진의 공통적인 최적화 방법입니다.</p>
        ...
        <p class="subtitle fragment fade-in-then-out">parser에서 분해된 token을 기반으로 나무구조를 만듭니다.</p>
        <p class="subtitle fragment fade-in-then-out">인터프리터는 코드를 한줄한줄 읽어가며 최적화 되지 않은</p>
        <p class="subtitle fragment fade-in-then-out">bytecode를 빠르게 생성합니다.</p>
        ...
        <p class="subtitle fragment fade-in-then-out">그리고 bytecode와 함께 optimizing compiler로 보내집니다.</p>
        <p class="subtitle fragment fade-in-then-out">optimizing compiler에서는 프로파일링된 데이터를 기반으로</p>
        <p class="subtitle fragment fade-in-then-out">최적화된 nativecode를 생성합니다.</p>
        <p class="subtitle fragment fade-in-then-out">만약 정확하지 않은 결과가 나오면 다시 deoptimize하여 bytecode로 되돌립니다.</p>
      ...
      <section data-title="2.JS Engine 작동원리(V8)">
        ...
        <p class="subtitle fragment fade-in-then-out">이렇게 모인 바이트코드와 프로파일링 데이터를 TurboFan이라고 부르는 최적화 컴파일러로 보내서 </p>
        <p class="subtitle fragment fade-in-then-out">프로파일링 된 데이터를 기반으로 매우 최적화된 기계어 코드를 만들어냅니다. </p>
        ...
        <p class="subtitle fragment fade-in-then-out">시간이 조금 더 걸리더라도 효율적인 nativecode를 생산하느냐 아니면 시간을 조금 더 아껴 덜 효율적인 nativecode를 생산하느냐의 차이입니다.</p>
        <p class="subtitle fragment fade-in-then-out">엔진마다 최적화 과정은 조금씩 다르지만 파서,인터프리터,컴파일러가 있는 동일한 아키텍쳐로 구성되어 있습니다.</p>
        <p class="subtitle fragment fade-in-then-out">그러면 여태 말해왔던 인터프리터가 변환하는 bytecode와 JITC가 만들어낸 nativecode가 어떻게 생겼는지 잠시 보겠습니다.</p>
      </section>
      <section data-title="2.JS Engine 작동원리">
      ...
      <section data-title="2.JS Engine 작동원리">
        ...
        <p class="subtitle fragment fade-in-then-out">이번 코드는 V8의 최적화 컴파일러인 TurboFan이 최적화하는 모습을 보실수 있습니다.</p>
        <p class="subtitle fragment fade-in-then-out">sample 함수는 그냥 인자를 받아서 변형하고 걸러내고</p>
        <p class="subtitle fragment fade-in-then-out">순서를 뒤집어서 반환하는 역할을 수행합니다.</p>
        <p class="subtitle fragment fade-in-then-out">여기서 Turbofan이 감시하고 있는 최적화 대상은 sample, map,</p>
      ...
      <section data-title="3.최적화의 조건">
        ...
        <p class="subtitle fragment fade-in-then-out">javascript는 동적 타입 언어이기 때문에 객체를 생성할때 메모리를 얼마나 할당해야하는지 모릅니다.</p>
        ...
        <p class="subtitle fragment fade-in-then-out">javascript내에서 dictionary에 정의된 속성값을 찾는것은 느리기 때문에,</p>
      ...
      <section data-title="3.최적화의 조건">
        ...
        <p class="subtitle fragment fade-in-then-out">동적인 언어라고 해도 실제로는 안바뀌고 성능을 빠르게하려면 반복문을 최적화 해야한다는 가정이 바탕에 깔려있습니다.</p>
      </section>
      <section data-title="3.최적화의 조건">
        <h3>Inline Caching</h3>
        <pre><code class="javascript" data-trim>
        ...
        <p class="subtitle fragment fade-in-then-out">하나의 배열 안에는 모드 같은 필드 구조를 가지는 객체들만 넣어서 접근해야합니다.</p>
      ...
      <section data-title="4.처리성능 향상 방법">
        ...
        <p class="subtitle fragment fade-in-then-out">앞서 V8 엔진의 최적화 방법을 장황하게 설명했지만</p>
        <p class="subtitle fragment fade-in-then-out">엔진 입장에서의 최적화된 코드는</p>
        <p class="subtitle fragment fade-in-then-out">자바스크립트를 동적인 언어라고 생각하며 사용해서</p>
        <p class="subtitle fragment fade-in-then-out">hidden class나 inline caching의 이점을 최대한 활용하는 방법을 추천하고 싶습니다.</p>
      </section>
      <section>
        <h3>감사합니다.</h3>
      </section>
